...
void __baseModel::addObserver(std::shared_ptr<ICliViewObserver> observer) 
{
    __observer.emplace_back(observer);
}

void __baseModel::removeObserver(std::shared_ptr<ICliViewObserver> observer)
{
    auto it = std::find_if(__observer.cbegin(), __observer.cend(), [&](const std::weak_ptr<ICliViewObserver>& sub){
            auto itr = sub.lock();
            return itr == observer;
        });
